...
class Zakra_Customize_Base_Option {
	...
	public function zakra_customizer_options( $wp_customize ) {
		...
		foreach ( $this->elements as $el_key => $el_data ) :
			...
			if ( isset( $control_args['active_callback'] ) && is_array( $control_args['active_callback'] ) ) {
				...
				if ( 'zakra_pro' === substr( $cb_setting_id, 0, 9 ) && function_exists( 'zakra_pro_options' ) ) {
					$this->ac_default[] = $wp_customize->get_setting( 'zakra_pro[' . $cb_setting_id . ']' )->default;
	...
	public function evaluate() {
		foreach ( $this->active_callback_old[ ( $this->count_evaluate ) ] as $count => $ruleset ) :
			...
			if ( 'zakra_pro' === substr( $ac_setting_id, 0, 9 ) && function_exists( 'zakra_pro_options' ) ) {
				$option_val = zakra_pro_options( $ac_setting_id, 'text_html' );
			} else {
		...
		foreach ( $this->elements as $el_key => $el_data ) :
			...
			if ( isset( $el_data['output'] ) ) {
				...
				foreach ( $output as $output_key => $output_val ) :
					...
					if ( 'zakra_pro' === substr( $el_key, 0, 9 ) && function_exists( 'zakra_pro_options' ) ) {
						$key    = str_replace( array( 'zakra_pro[', ']' ), '', $el_key );
						$el_val = zakra_pro_options( $key, $default );
					} else {
