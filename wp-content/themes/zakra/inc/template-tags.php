...
if ( ! function_exists( 'zakra_posted_on' ) ) :
	...
	function zakra_posted_on() {
		...
		/* translators: %s: post date. */
			esc_html_x( 'Posted on %s', 'post date', 'zakra' ),
...
if ( ! function_exists( 'zakra_posted_by' ) ) :
	...
	function zakra_posted_by() {
		...
		/* translators: %s: post author. */
			esc_html_x( 'By %s', 'post author', 'zakra' ),
	...
	function zakra_posted_in() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'zakra' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
		}
	}
endif;
	...
	function zakra_post_tags() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'zakra' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
